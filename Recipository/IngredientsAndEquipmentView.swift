...
struct IngredientsAndEquipmentView: View {
    ...
    private var rows: [IngredientDisplayRow] {
        meal.ingredientPairs.enumerated().map { index, pair in
            IngredientDisplayRow(
                id: index,
                ingredientName: pair.ingredient,
    ...
    private var equipment: [(emoji: String, name: String)] {
        EquipmentSuggestions.items(for: meal)
    }
...
private enum EquipmentSuggestions {
    static func items(for meal: Meal) -> [(emoji: String, name: String)] {
        ...
        let cat = meal.strCategory?.lowercased() ?? ""
        let title = meal.strMeal.lowercased()
